...
#include <stdlib.h>// Notes Frequency from http://www.phy.mtu.edu/~suits/notefreqs.html
...
#define A5  880
#define Ac5 932
#define B5  988#define C6  1047volatile unsigned char duty_cyc_a,duty_cyc_b, duty_cyc_c,led_a,led_b,led_c;
volatile unsigned int tempo;// TIMER1 Overflow Interrupt
ISR(TIMER1_OVF_vect)
{
...
void PlayNotes(unsigned int note_frequency,unsigned int duration)
{
   ...
   topvalue=(F_CPU / (16 * note_frequency));   // Reset the TIMER1 16 bit Counter
   TCNT1H = 0;
   TCNT1L = 0;
   ...
   duty_cycle=top_value / 2;   OCR1AH=(duty_cycle >> 8 ) & 0x00FF;
...
}int main(void)
{
  ...
  led_c=1;  sei();               // Enable Interrupt  song_index=0;
  tempo=0;
  ...
  for(;;) {            // Loop Forever
	...
	PlayNotes(C5,1050 + tempo);	PlayNotes(C5,550 + tempo); PlayNotes(C5,175 + tempo);
	...
	PlayNotes(C5,200); PlayNotes(D5,200 + tempo);	if (song_index >= 1) {
	  PlayNotes(C5,1600 + tempo);	  if (song_index == 3) {
	    _delay_ms(100 + tempo);
      ...
      _delay_ms(100 + tempo);      PlayNotes(C5,450 + tempo); PlayNotes(D5,150 + tempo);
      ...
      PlayNotes(E5,1 + tempo); PlayNotes(E5,1000 + tempo);      PlayNotes(C5,350 + tempo); PlayNotes(D5,250 + tempo);
      ...
      PlayNotes(A5,450 + tempo); PlayNotes(A5,100 + tempo);      PlayNotes(E5,150 + tempo); PlayNotes(G5,1000 + tempo);
      ...
      PlayNotes(D5,1300 + tempo);      song_index =3;
    }
  }
